...
// export const renderScope = (root: HTMLElement, name: string, api: any) => {
//   const camelizedName = name.replace(/(^|-)([a-z])/g, (_match, _prefix, letter) => letter.toUpperCase());

//   const part = root.querySelector<HTMLElement>(`[data-scope='${name}']`);
...
//     if (childrenValue) {
//       part.textContent = childrenValue; // Set the text content of the element
//     }
//   }
// }
...
export const renderPart = (root: HTMLElement, name: string, api: any) => {
  ...
  if (part && typeof api[getterName] === "function") {
    ...
    console.warn(`Part "${name}" or method "${getterName}" is missing in API.`);
  }
};
...
export const getString = (el: HTMLElement, name: string) => {
  // const kebabName = name.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase();
  return el.dataset[name];
};


export const getElement = (el: HTMLElement, name: string): (() => HTMLElement | null) | undefined => {
  const attributeValue = el.dataset[name]; // Get the value of the `data-*` attribute
  if (!attributeValue) return undefined; // Return undefined if the attribute is not set

  // Return a function that queries the element with the ID from the attribute
  return () => document.querySelector(`#${attributeValue}`);
};



export const getBoolean = (el: HTMLElement, name: string) => {
...
export const getAttributes = (root: HTMLElement, name: string) => {
  ...
  const attrs: Array<{ name: string; value: string }> = []; // Explicitly typing attrs
